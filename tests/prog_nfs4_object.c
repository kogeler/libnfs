...
{
        char request[8192], reply[8192], attributes[12];
        uint32_t marker, size, bitmap = (1U << FATTR4_TYPE) | (1U << FATTR4_SIZE);
        ...
        for (unsigned i = 0; i < expected_count; i++) {
                ...
                case OP_GETATTR: {
                        ...
                        attr->attr_vals.attrlist4_len = malformed ? 4 : sizeof(attributes);
                        break;
                }
                case OP_OPEN:
...
int main(void)
{
        ...
        fail_op = OP_PUTFH;
        cb = (struct completion){.kind = 2};
        RUN(nfs4_object_getattr_async(context, file, complete, &cb), -ESTALE, OP_GETATTR);
        cb = (struct completion){.kind = 3};
        RUN(nfs4_object_open_async(context, file, O_RDONLY, complete, &cb), -ESTALE, OP_OPEN, OP_GETFH);
        fail_op = 0;
